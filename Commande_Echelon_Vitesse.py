...
def mainVitesse(vitesseConsigne):
    ...
    MyEpos.setOperationMode(c_int(-2),pErrorCode_i)
    ...
    while (toc() < dureeExp):
        ...
        if ((pPositionIs_i.contents.value/qc2mm) > 490)  :
             if vitesseConsigne >0 :        
                 MyEpos.setVelocityMust(c_long(0),pErrorCode_i)
                 print("Le bras ne peut pas monter car il va taper la butée !!!")
                 break
             else : 
                 MyEpos.setVelocityMust(c_long(vitesseConsigne),pErrorCode_i)
        ...
        TabPosition.append(float(pPositionIs_i.contents.value)/float(qc2mm))
